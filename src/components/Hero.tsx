  ...
  return (
    <section ref={ref} className="relative min-h-screen flex items-center overflow-hidden pt-20">
      ...
      <div className="relative z-10 container mx-auto px-6 py-16 lg:py-24">
        ...
        >
          {/* Location badge */}
          <motion.div variants={item} className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-white/10 border border-white/20 text-white text-sm font-medium mb-8 backdrop-blur-sm">
          ...
          <motion.p variants={item} className="text-lg text-white/80 leading-relaxed mb-10 max-w-lg">
          ...
          <motion.div variants={item} className="flex flex-wrap gap-2 mb-10">
            {badges.map(({ icon: Icon, text }, i) => (
              <motion.div
                ...
                className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/10 border border-white/20 backdrop-blur-sm text-xs font-semibold text-white"
              >
          ...
          <motion.div variants={item} className="flex flex-col sm:flex-row gap-4 mb-14">
            <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}>
              <Link
                ...
                className="group flex items-center justify-center gap-2 px-8 py-4 bg-primary text-white rounded-full font-bold shadow-xl shadow-primary/30 hover:bg-primary/90 transition-colors"
              >
                Book Consultation
            ...
            <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}>
              <Link
                href="#specialties"
                className="flex items-center justify-center gap-2 px-8 py-4 border border-white/30 text-white rounded-full font-bold hover:bg-white/10 transition-colors backdrop-blur-sm"
              >
                Our Specialties
              </Link>
            </motion.div>
          ...
          {/* Stats */}
          <motion.div variants={item} className="grid grid-cols-3 gap-6 pt-8 border-t border-white/20 max-w-sm">
            {stats.map((s) => (
              <div key={s.label}>
                <div className="text-2xl sm:text-3xl font-serif font-bold text-white">{s.value}</div>
                <div className="text-[10px] sm:text-xs text-white/60 uppercase tracking-widest font-semibold mt-1">{s.label}</div>
              </div>
            ))}
          </motion.div>
        </motion.div>
